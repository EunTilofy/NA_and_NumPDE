...
class SparseMat {
    ...
    public:
        ...
        Type& operator [] (const int& pos) { 
            ...
            for(pair<int, Type>& val : a[pos%5]) if(val.first == pos) return val.second;
            a[pos%5].push_back({pos, 0});
            return a[pos%5].back().second;
        }
        const Type& operator [] (const int& pos) const { 
            assert(pos <= len && pos >= 0);
            for(const pair<int, Type>& val : a[pos%5]) if(val.first == pos) return val.second;
            return zero;
        }
        void setv(int pos, Type v) {
            assert(pos <= len && pos >= 0);
            for(pair<int, Type>& val : a[pos%5]) if(val.first == pos) {val.second = v; return;}
            a[pos%5].push_back({pos, v});
        }
        Type v(int pos) {
            assert(pos <= len && pos >= 0);
            for(pair<int, Type>& val : a[pos%5]) if(val.first == pos) return val.second;
