 ...
 * 5. In ver3.0 I add 2d-function abstract class.
...
public:
    ...
    Vec<Ytype> grad (const Xtype& x, const Xtype &y) const {
        Vec<Ytype> ret(2);
        ret[0] = ((*this)(x+EPS_MIN, y) - (*this)(x-EPS_MIN, y))/(2*EPS_MIN);
        ret[0] = ((*this)(x, y+EPS_MIN) - (*this)(x, y-EPS_MIN))/(2*EPS_MIN);
        return ret;
    }
    Ytype div (const Xtype &x, const Xtype &y) const {
        return sum(grad(x, y));
    }
    Ytype lap (const Xtype &x, const Xtype &y) const {
        return ((*this)(x+EPS_MIN,y)+(*this)(x-EPS_MIN,y)
               +(*this)(x,y+EPS_MIN)+(*this)(x,y-EPS_MIN)
               -4*(*this)(x,y))/(EPS_MIN * EPS_MIN);
    }
};
